...
const VendorToCustomers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (sharedKey && jwt) {
        try {
          const encryptedPayload = await encryption({ data: { sourceRole: "Vendor", destinationRole: "Customer" } }, sharedKey);
      ...
      <div className="flex min-h-screen bg-gray-100">
        ...
        <div className="flex-1 p-6 ml-64">
          <Card className="bg-white shadow-md">
            <CardHeader>
              <CardTitle>Vendor to Customers</CardTitle>
              <CardDescription>Transactions from vendors to customers</CardDescription>
            </CardHeader>
            <CardContent>
...
export const LatestVendorToCustomers: React.FC = () => {
  const { sharedKey, jwt } = useKeyContext(); // Get sharedKey and jwt from context
  ...
  return (
    <Link href="/Dashboard/vendor-to-customers">
...
export default VendorToCustomers;
